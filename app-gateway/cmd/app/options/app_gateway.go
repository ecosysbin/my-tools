...
import (
	...
	configv1 "gitlab.datacanvas.com/aidc/app-gateway/pkg/apis/config/app_gateway/v1"
)
...
func (o *VclusterGatewayOptions) Validate() []error {
	...
	if o.ConfigFilePath == "" {
		if o.Server.TokenKey == "" {
			errs = append(errs, errors.New("server TokenKey is must"))
		}

		if o.Server.RespCacheKey == "" {
			errs = append(errs, errors.New("server RespCacheKey is must"))
		}
		if o.Casdoor.Endpoint == "" {
			errs = append(errs, errors.New("casdoor Endpoint is must"))
		}

		if o.Casdoor.ClientId == "" {
			errs = append(errs, errors.New("casdoor ClientId is must"))
		}

		if o.Casdoor.ClientSecret == "" {
			errs = append(errs, errors.New("casdoor ClientSecret is must"))
		}

		if o.Casdoor.OrganizationName == "" {
			errs = append(errs, errors.New("casdoor OrganizationName is must"))
		}

		if o.Casdoor.ApplicationName == "" {
			errs = append(errs, errors.New("casdoor ApplicationName is must"))
		}

		if o.Casdoor.Certificate == "" {
			errs = append(errs, errors.New("casdoor Certificate is must"))
		}
	}
