...
// PauseVClusterWithCleanup pauses a running vcluster and cleans up related Pods
func PauseVClusterWithCleanup(ctx context.Context, kubeClient *kubernetes.Clientset, vClusterId, namespace string, log log.BaseLogger) error {
	// Pause the vcluster
	err := PauseVCluster(ctx, kubeClient, vClusterId, namespace, log)
	if err != nil {
		return err
	}

	// Clean up the vcluster's Pods
	err = DeletePods(ctx, kubeClient, fmt.Sprintf("%s=%s", vClusterManagedByLabel, vClusterId), namespace, log)
	if err != nil {
		return err
	}

	// Clean up multi-namespace vcluster workloads
	err = DeleteMultiNamespaceVClusterWorkloads(ctx, kubeClient, vClusterId, namespace, log)
	if err != nil {
		return err
	}

	log.Infof("Successfully paused vcluster %s/%s and cleaned up related Pods", vClusterId, namespace)
	return nil
}
